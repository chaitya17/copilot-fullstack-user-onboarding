...
wait_for() {
    ...
    do
        if [[ $PROTOCOL == "tcp" ]]; then
            result=$(timeout 1 bash -c "cat < /dev/null > /dev/tcp/$HOST/$PORT" 2>/dev/null && echo "open" || echo "closed")
        else
            result=$(timeout 1 bash -c "cat < /dev/null > /dev/udp/$HOST/$PORT" 2>/dev/null && echo "open" || echo "closed")
        fi
        if [[ $result == "open" ]]; then
            end_ts=$(date +%s)
...
if [[ $CLI != "" ]]; then
    if [[ $RESULT -ne 0 && $STRICT -eq 1 ]]; then
        echoerr "$HOST:$PORT is not available, not executing command"
        exit $RESULT
    fi
    exec "${CLI[@]}"
else
